...
export function previewToWorkflowDraft(
  ...
  const steps: WorkflowStepDraft[] = preview.steps.map((s) => {
    ...
    return {
      ...
      donViXuLyId: s.donViXuLyId ?? undefined,
      vaiTroXuLyId: s.vaiTroXuLyHoSoId ?? undefined,
      slaNgay: s.soNgayLapHoSo,
      loaiThoiHan: mapLoaiHanToUi(s.loaiHan),
      coKyDuyet: s.vaiTroKyDuyetId != null,
      loaiThoiHanKyDuyet: mapLoaiHanToUi(s.loaiHanKyDuyet),
      ...
      donViKyDuyetId: s.donViKyHoSoId ?? undefined,
      vaiTroKyDuyetId: s.vaiTroKyDuyetId ?? undefined,
