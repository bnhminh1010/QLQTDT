...
import type { PhongFormValues } from "./types";
        ...
        <form onSubmit={handleSubmit(onSave)} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input className={errors.ma ? inputErrCls : inputCls} placeholder="VD: KUB"
                {...register("ma", {
                  required: "Vui lòng nhập mã",
                  validate: {
                    noBlank: v => v.trim().length > 0 || "Mã không được để trống",
                    noSpecial: v => /^[A-Za-z0-9_-]+$/.test(v.trim()) || "Mã chỉ chứa chữ, số, _ hoặc -",
                    unique: v => !existingIds.includes(v.trim().toUpperCase()) || "Mã đã tồn tại",
                  },
                  maxLength: { value: 20, message: "Mã tối đa 20 ký tự" },
                })} />
              {errors.ma && <p className="text-xs text-red-500 mt-1">{errors.ma.message}</p>}
            </div>
            <div>
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <button type="submit" disabled={isSubmitting} className="h-9 px-5 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white text-sm font-semibold rounded-xl flex items-center gap-2">
